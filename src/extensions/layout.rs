...
// Disabled: syntax_highlighting module
...
pub enum UiLayout {
    ...
    /// [`crate::components::list_item::ListItem`] with [`crate::components::list_item::PropertyContent`]. Text should
    /// truncate.
    List,
...
impl UiLayout {
    ...
    ///
    /// Simplified version without syntax highlighting support.
    ...
    pub fn data_label(
        ...
        self.data_label_impl(ui, layout_job)
    }
    ...
    fn decorate_url(ui: &mut egui::Ui, mut galley: Arc<egui::Galley>) -> egui::Response {
        ...
        if text.contains("://") {
            // Trim quotes if present (simplified version without syntax highlighting)
            const QUOTE_CHAR: char = '"';
            let stripped = text.trim_matches(QUOTE_CHAR);
