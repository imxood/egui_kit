...
use crate::extensions::egui_ext::context_ext::ContextExt as _;
...
impl UICommand {
    ...
    pub fn icon(self) -> Option<&'static crate::Icon> {
        match self {
            Self::OpenWebHelp => Some(&crate::components::basic::icon::EXTERNAL_LINK),
            Self::OpenRerunDiscord => Some(&crate::components::basic::icon::DISCORD),
            _ => None,
        }
    }
    ...
    pub fn tooltip_ui(self, ui: &mut egui::Ui) {
        ...
        if let Some(shortcut) = self.primary_kb_shortcut(os) {
            crate::components::Help::new_without_title()
                .control(label, crate::components::basic::IconText::from_keyboard_shortcut(os, shortcut))
                .ui(ui);
        } else {
            ui.label(label);
        }
