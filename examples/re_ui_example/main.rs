// Run the example with `cargo r -p egui_kit --example egui_kit_example`
...
use egui_kit::components::filter::{FilterState, format_matching_text};
use egui_kit::components::list_item::ListItemContentButtonsExt as _;
use egui_kit::components::notifications::NotificationUi;
use egui_kit::extensions::egui_ext::OnResponseExt as _;
use egui_kit::{
    CommandPalette, CommandPaletteAction, CommandPaletteUrl, ContextExt as _, DesignTokens,
    UICommand, UICommandSender, UiExt as _, components::Help, components::IconText,
    components::basic::icon, list_item,
};
...
fn main() -> eframe::Result {
    // Simple logging to console (no logger initialization needed)

    let fullsize_content = egui_kit::fullsize_content(os::OperatingSystem::default());
    let native_options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            .with_app_id("egui_kit_example")
            .with_decorations(!egui_kit::CUSTOM_WINDOW_DECORATIONS) // Maybe hide the OS-specific "chrome" around the window
            .with_fullsize_content_view(fullsize_content)
            .with_inner_size([1200.0, 800.0])
            .with_title_shown(!fullsize_content)
            .with_titlebar_buttons_shown(!egui_kit::CUSTOM_WINDOW_DECORATIONS)
            .with_titlebar_shown(!fullsize_content)
            .with_transparent(egui_kit::CUSTOM_WINDOW_DECORATIONS), // To have rounded corners without decorations we need transparency

        ..Default::default()
    };

    eframe::run_native(
        "egui_kit example app",
        native_options,
        Box::new(move |cc| {
            egui_kit::apply_style_and_install_loaders(&cc.egui_ctx);
...
pub struct ExampleApp {
    ...
    // Removed: text_log_rx (ChannelLogger not available)
    tree: egui_tiles::Tree<Tab>,

    /// regular modal
    modal_handler: egui_kit::components::modal::ModalHandler,

    /// modal with full span mode
    full_span_modal_handler: egui_kit::components::modal::ModalHandler,
...
impl ExampleApp {
    fn new(ctx: egui::Context) -> Self {
        // Removed: ChannelLogger initialization (not available in standard log crate)
        // Log messages will be printed to console via env_logger
    ...
    // Removed: show_text_logs_as_notifications() method
    // (ChannelLogger not available in standard log crate)
    // Log messages from the "Log info/warn/error" buttons will be printed to console
}

impl eframe::App for ExampleApp {
    fn clear_color(&self, _visuals: &egui::Visuals) -> [f32; 4] {
        if egui_kit::CUSTOM_WINDOW_DECORATIONS {
            [0.0; 4] // transparent
        } else {
    ...
    fn update(&mut self, egui_ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // Removed: show_text_logs_as_notifications() call
        ...
        let left_panel_bottom_section_ui = |ui: &mut egui::Ui| {
            ...
            self.modal_handler.ui(
                ui.ctx(),
                || egui_kit::components::modal::ModalWrapper::new("Modal window"),
            ...
            ui.section_collapsing_header("Data")
                .with_button(
                    ui.small_icon_button_widget(&egui_kit::components::basic::icon::ADD, "Add")
                        .on_menu(|ui| {
                            ui.weak("empty");
                        }),
                )
        ...
        if let Some(cmd) = egui_kit::UICommand::listen_for_kb_shortcut(egui_ctx) {
    ...
    fn top_bar(&mut self, frame: &eframe::Frame, egui_ctx: &egui::Context) {
        ...
        egui::TopBottomPanel::top("top_bar")
            ...
            .show(egui_ctx, |ui| {
                ...
                if !egui_kit::native_window_bar(egui_ctx.os()) {
    ...
    fn top_bar_ui(&mut self, ui: &mut egui::Ui) {
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            ...
            ui.medium_icon_toggle_button(
                &egui_kit::components::basic::icon::RIGHT_PANEL_TOGGLE,
                "Selection panel toggle",
                &mut self.show_right_panel,
            );
            ui.medium_icon_toggle_button(
                &egui_kit::components::basic::icon::BOTTOM_PANEL_TOGGLE,
                "Time panel toggle",
                &mut self.show_bottom_panel,
            );
            ui.medium_icon_toggle_button(
                &egui_kit::components::basic::icon::LEFT_PANEL_TOGGLE,
...
impl egui_tiles::Behavior<Tab> for MyTileTreeBehavior {
        ...
        egui::Frame::new().inner_margin(4.0).show(ui, |ui| {
            ...
            ui.label("Help").on_hover_ui(|ui| {
                Help::new("Help example")
                    ...
                    .control("Pan", (icon::LEFT_MOUSE_CLICK, "+", "drag"))
                    .control(
                        "Zoom",
                    ...
                    .control("Reset view", ("double", icon::LEFT_MOUSE_CLICK))
                    .ui(ui);
            });
    ...
    fn tab_bar_height(&self, style: &egui::Style) -> f32 {
        egui_kit::design_tokens_of_visuals(&style.visuals).title_bar_height()
    }
